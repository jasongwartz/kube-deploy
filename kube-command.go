...
func canarySimmer(waitTimeSeconds int) (bool) {
...
func kubeInstantRollback() {
	...
	if len(rollbackTargets.Items) != 1 || len(isLiveDeployments.Items) != 1 {
		fmt.Println("=> Whoah, there's more than one 'is_live' deployment or 'rollback-target' deployment. You should fix that first.")
